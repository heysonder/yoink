...
export default function FormatToggle({ value, onChange, disabled }: FormatToggleProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {isLossless && (
        <span
          className={`flex items-center gap-1 text-[9px] tracking-wider animate-fade-in ${value === "alac" ? "text-pink/40" : "text-mauve/40"}`}
          style={{ opacity: 0 }}
        >
          {value === "alac" ? (
            <>
              <svg className="w-2.5 h-2.5" viewBox="0 0 384 512" fill="currentColor">
